	...
	dup_getData_d3: function(obj){
		if(this.processing==1){
			...
			this.get(obj.href, function(data){
				...
				dup_output += '<div style="clear:both">Автор' + (dup_posts ? ' <b>'+ dup_posts+'</b> '+ (dup_posts % 10 == 1 && dup_posts % 100 != 11 ? 'поста':'постов') : '') + (dup_posts && dup_comments ? ' и':'') + (dup_comments ? ' <b>'+dup_comments+'</b> ' + (dup_comments % 10 == 1 && dup_comments % 100 != 11 ? 'комментария':'комментариев') : '')  + '<br>Всего подписчиков: '+dup_subscribers+'</div>';
