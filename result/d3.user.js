...
{d3.content.addConfigLink('configLink');$j('#configLink').click(function(event){d3.config.getBox().show();return false;});},getBox:function()
{if(this.box==undefined)
this.box=this.drawBox();return this.box;},controlName:function(module,control){return'A'+Math.abs(crc32(module))+'_'+control;},drawBox:function()
{var box=$j(d3.newElement('div',{style:{position:'fixed',top:($j(window).height()/2-150)+'px',left:($j(window).width()/2-300)+'px',width:'600px',height:'300px',backgroundColor:'#FFFFCC',border:'1px grey solid',zIndex:'1000',padding:'0 1ex'},attributes:{id:'configBox'},innerHTML:'Настройки сервис-пака <a id="configCloser" style="position:absolute;right:1ex;top:0px;cursor:pointer">закрыть</a><div id="configTabs"></div>'}));$j('body').append(box);$j('#configCloser').click(function(event){box.hide();d3.config.readAndSave();return false;});this.tabs=new TabSheets($j('#configTabs'));for(var sheetName in this.controls)
this.tabs.addSheet(sheetName,this.drawSheet(this.controls[sheetName]));return box;},drawSheet:function(data)
{var html='<table style="width: 100%; border:0px; border-collapse:collapse; padding: 0px"><col width="24"></col><tbody>';for(var id in data){if(data[id].name=='active'){html+='<tr><td colspan="2" style="height: 20px;"><div style="padding-top:8px;"><div style="height: 1px; background-color: black;"><span style="background-color: white; position: relative; top: -0.8em; left: 20px;">Модуль "'+data[id].caption+'"</span></div></div></td></tr>';data[id].caption="Активировать";}
html+=data[id].draw();}
return html+'</tbody></table>';},Control:function(module,name)
{this.module=module;this.name=name;module.config[this.name]._control=this;this.setValue=function(newValue){d3.config.data[this.id]=this.module.config[this.name].value=this.value=newValue;};this.getValue=function(){return $j('#'+this.id).val();};this.update=function(){this.setValue(this.getValue());};for(var i in module.config[name])
this[i]=module.config[name][i];for(i in d3.config[this.type])
this[i]=d3.config[this.type][i];this.id=d3.config.controlName(module.name,name);if(this.caption==undefined)
this.caption=name=='active'?module.name:name;if(d3.config.data[this.id]!=undefined)
this.setValue(d3.config.data[this.id]);},checkbox:{draw:function()
{return'<tr><td><input type="checkbox" id="'+this.id+'" name="'+this.id+'"'+(this.value?' checked':'')+'></td><td><label for="'+this.id+'">'+this.caption+'</label></td></tr>';},getValue:function(){return $j('#'+this.id).attr('checked')?true:false;}},text:{draw:function()
{return'<tr><td colspan="2"><label>'+this.caption+'<input type="text" id="'+this.id+'" name="'+this.id+'" value="'+this.value+'"></label></td></tr>';}},html:{draw:function()
{return'<tr><td colspan="2">'+this.value+'</td></tr>';},getValue:function(){return this.module.config[this.name].value;}},hidden:{draw:function(){return'';},getValue:function(){return this.module.config[this.name].value;}},radio:{draw:function()
{if(this.columns==undefined)
{this.columns=0;for(var i in this.options)this.columns++;this.columns=Math.floor(Math.sqrt(this.columns));}
var html='<tr><td colspan="2">'+this.caption+'</td></tr><tr><td></td><td><table><tbody><tr>';var counter=1;for(var i in this.options)
{if((++counter%this.columns)==0&&counter>2)html+='</tr><tr>';html+='<td><label><input type="radio" name="'+this.id+'" value="'+this.options[i]+'"'+(this.options[i]==this.value?' checked':'')+'> '+i+'</label></td>';}
return html+'</tr></tbody></table></td></tr>';},getValue:function(){return $j('input:radio[name='+this.id+']:checked').val();}},select:{draw:function()
{var html='<tr><td colspan="2">'+this.caption+' <select name="'+this.id+'" id="'+this.id+'">';for(var i in this.options)
{html+='<option value="'+this.options[i]+'"'+(this.options[i]==this.value?' selected':'')+'> '+i+'</option>';}
return html+'</select></td></tr>';},getValue:function(){return $j('select#'+this.id+' option:selected').val();}},readAndSave:function()
{for(var sheet in this.controls)
for(var id in this.controls[sheet])
this.controls[sheet][id].update();this.save();},save:function()
{localStorage.setItem('dirtySpm',d3.json.encode(this.data));},load:function()
{var saved=localStorage.getItem('dirtySpm');if(saved!==null)
this.data=d3.json.decode(saved);},addModule:function(module)
{if(module.config==undefined)return;if(this.controls[module.type]==undefined)
this.controls[module.type]={};for(var i in module.config)
{var control=new this.Control(module,i);this.controls[module.type][control.id]=control;}}},initCore:function()
{this.getOriginal();this.initContentModules();this.user=this.content.findUser();this.config.load();this.addModule(d3.config);this.window.d3=this;},getOriginal:function()
{if($j.browser.webkit)
{var retval=d3.newElement('div',{attributes:{onclick:"return {window:window,document:document};"}}).onclick();this.window=retval.window;this.document=retval.document;}
else if($j.browser.mozilla)
{this.window=unsafeWindow;this.document=document.wrappedJSObject;}
else if($j.browser.opera)
{this.window=window;this.document=document;}else
alert("Don't know method to get original window for this browser");},xpath:{get:function(selector,context)
{if(context==undefined)context=document;return document.evaluate(selector,context,document.createNSResolver(document),XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null);},each:function(selector,handler,context)
{var nodes=this.get(selector,context);for(var i=0;i<nodes.snapshotLength;i++)
if(handler(nodes.snapshotItem(i),i)===false)return false;return true;}}};(function(){var table="00000000 77073096 EE0E612C 990951BA 076DC419 706AF48F E963A535 9E6495A3 0EDB8832 79DCB8A4 E0D5E91E 97D2D988 09B64C2B 7EB17CBD E7B82D07 90BF1D91 1DB71064 6AB020F2 F3B97148 84BE41DE 1ADAD47D 6DDDE4EB F4D4B551 83D385C7 136C9856 646BA8C0 FD62F97A 8A65C9EC 14015C4F 63066CD9 FA0F3D63 8D080DF5 3B6E20C8 4C69105E D56041E4 A2677172 3C03E4D1 4B04D447 D20D85FD A50AB56B 35B5A8FA 42B2986C DBBBC9D6 ACBCF940 32D86CE3 45DF5C75 DCD60DCF ABD13D59 26D930AC 51DE003A C8D75180 BFD06116 21B4F4B5 56B3C423 CFBA9599 B8BDA50F 2802B89E 5F058808 C60CD9B2 B10BE924 2F6F7C87 58684C11 C1611DAB B6662D3D 76DC4190 01DB7106 98D220BC EFD5102A 71B18589 06B6B51F 9FBFE4A5 E8B8D433 7807C9A2 0F00F934 9609A88E E10E9818 7F6A0DBB 086D3D2D 91646C97 E6635C01 6B6B51F4 1C6C6162 856530D8 F262004E 6C0695ED 1B01A57B 8208F4C1 F50FC457 65B0D9C6 12B7E950 8BBEB8EA FCB9887C 62DD1DDF 15DA2D49 8CD37CF3 FBD44C65 4DB26158 3AB551CE A3BC0074 D4BB30E2 4ADFA541 3DD895D7 A4D1C46D D3D6F4FB 4369E96A 346ED9FC AD678846 DA60B8D0 44042D73 33031DE5 AA0A4C5F DD0D7CC9 5005713C 270241AA BE0B1010 C90C2086 5768B525 206F85B3 B966D409 CE61E49F 5EDEF90E 29D9C998 B0D09822 C7D7A8B4 59B33D17 2EB40D81 B7BD5C3B C0BA6CAD EDB88320 9ABFB3B6 03B6E20C 74B1D29A EAD54739 9DD277AF 04DB2615 73DC1683 E3630B12 94643B84 0D6D6A3E 7A6A5AA8 E40ECF0B 9309FF9D 0A00AE27 7D079EB1 F00F9344 8708A3D2 1E01F268 6906C2FE F762575D 806567CB 196C3671 6E6B06E7 FED41B76 89D32BE0 10DA7A5A 67DD4ACC F9B9DF6F 8EBEEFF9 17B7BE43 60B08ED5 D6D6A3E8 A1D1937E 38D8C2C4 4FDFF252 D1BB67F1 A6BC5767 3FB506DD 48B2364B D80D2BDA AF0A1B4C 36034AF6 41047A60 DF60EFC3 A867DF55 316E8EEF 4669BE79 CB61B38C BC66831A 256FD2A0 5268E236 CC0C7795 BB0B4703 220216B9 5505262F C5BA3BBE B2BD0B28 2BB45A92 5CB36A04 C2D7FFA7 B5D0CF31 2CD99E8B 5BDEAE1D 9B64C2B0 EC63F226 756AA39C 026D930A 9C0906A9 EB0E363F 72076785 05005713 95BF4A82 E2B87A14 7BB12BAE 0CB61B38 92D28E9B E5D5BE0D 7CDCEFB7 0BDBDF21 86D3D2D4 F1D4E242 68DDB3F8 1FDA836E 81BE16CD F6B9265B 6FB077E1 18B74777 88085AE6 FF0F6A70 66063BCA 11010B5C 8F659EFF F862AE69 616BFFD3 166CCF45 A00AE278 D70DD2EE 4E048354 3903B3C2 A7672661 D06016F7 4969474D 3E6E77DB AED16A4A D9D65ADC 40DF0B66 37D83BF0 A9BCAE53 DEBB9EC5 47B2CF7F 30B5FFE9 BDBDF21C CABAC28A 53B39330 24B4A3A6 BAD03605 CDD70693 54DE5729 23D967BF B3667A2E C4614AB8 5D681B02 2A6F2B94 B40BBE37 C30C8EA1 5A05DF1B 2D02EF8D";crc32=function(str,crc){if(crc==window.undefined)crc=0;var n=0;var x=0;crc=crc^(-1);for(var i=0,iTop=str.length;i<iTop;i++){n=(crc^str.charCodeAt(i))&0xFF;x="0x"+table.substr(n*9,8);crc=(crc>>>8)^x;}
return crc^(-1);};})();RegExp.escape=function(text){return text.replace(/[-[\]{}()*+?.,\\^$|#\s]/g,"\\$&");};var TabSheet=function(name,body,parent)
{this.name=name;this.body=body;this.parent=parent;this.tabNode=jQuery('<div class="tbsHeader">'+name+'</div>');this.tabNode.get(0).object=this;this.tabNode.click(function(event){parent.hideAll();this.object.show();});this.parent.tabs.append(this.tabNode);this.node=jQuery('<div class="tbsSheet">'+body+'</div>');this.hide();this.parent.node.append(this.node);};TabSheet.prototype={display:function(value){this.node.get(0).style.display=value;},hide:function(){this.display('none');this.tabNode.removeClass('tbsActive');},show:function(){this.display('inline-block');this.tabNode.addClass('tbsActive');}};var TabSheets=function(node)
{this.node=node;this.tabs=jQuery('<div class="tbsTabs"></div>');this.node.append(this.tabs);};TabSheets.prototype={sheets:[],addSheet:function(name,body)
{this.sheets.push(new TabSheet(name,body,this));if(this.sheets.length==1)
this.sheets[0].show();},hideAll:function(){this.sheets.forEach(function(item){item.hide();});}};jQuery('head').append('<style>\
.tbsTabs{float:left;}\
...
{this.container=container;container.get(0).comment=this;this.id=this.container.attr('id');this.isNew=this.container.hasClass('new');this.isMine=this.container.hasClass('mine');this.userId=parseInt(/u(\d+)/.exec(this.container.attr('class'))[1],10);this.indent=parseInt(/indent_(\d+)/.exec(this.container.attr('class'))[1],10);this.userName=$j('.c_user',this.container).text();};Comment.prototype=new Item
({contentClass:'.c_body',footerClass:'.c_footer',getAuthor:function(){return new User(this.userName,this.userId);},getClass:function(){return'comment';}});d3.Comment=Comment;d3.page={inbox:document.location.pathname.substr(0,10)=="/my/inbox/",onlyNew:(document.location.href.indexOf('#new')>-1),user:(window.location.pathname.indexOf("/user/")>=0)||(window.location.pathname.indexOf("/users/")>=0)};d3.get={logoutLink:function(){return $j('#js-header_logout_link');},leftNavigation:function(){return $j('#leftNavigator');},items:function(){return d3.content.items();}};d3.addContentModule(/(.*\.)?d3.ru/i,{type:"Ядро",author:'crimaniak',name:'Интерфейс к содержимому d3.ru',variant:'d3.ru',posts:[],comments:[],listeners:[],run:function()
...
me.listeners[i](comment);}});this.createLeftNavigator();},countItems:function()
...
processor(items[i]);this.onNewComment(processor);},addToHeaderNav:function(item,after)
{var nav=$j('#js-header_nav_user_menu');var list=$j('ul',nav);if(after!==undefined)
{after=$j('li:contains("'+after+'")',list);console.log(after);console.log(after.text());item.insertAfter(after);}else
list.append(item);nav.height(nav.height()+item.height());},addToLeftNav:function(item,after)
{var nav=$j('#leftNavigator');var list=$j('ul',nav);if(after!==undefined)
{after=$j('li:contains("'+after+'")',list);console.log(after);console.log(after.text());item.insertAfter(after);}else
list.append(item);},addConfigLink:function(id)
{this.addToLeftNav($j('<li class="b-header_nav_user_menu_item"><a href="#" id="'+id+'">сервис-пак</a></li>'));},findUser:function()
{return{id:0,name:d3.window.globals.current_user};},createLeftNavigator:function()
{$j('div.l-content_aside').append('<div id="leftNavigator"><ul style="list-style-type: none;"></ul></div>');}});d3.page={inbox:document.location.pathname.substr(0,10)=="/my/inbox/",onlyNew:(document.location.href.indexOf('#new')>-1),user:(window.location.pathname.indexOf("/user/")>=0)||(window.location.pathname.indexOf("/users/")>=0)};d3.get={logoutLink:function(){return $j('#js-header_logout_link');},leftNavigation:function(){return $j('.left_col_nav');},items:function(){return d3.content.items();}};d3.addContentModule(/(.*\.)?dirty.ru/i,{type:"Ядро",author:'crimaniak',name:'Интерфейс к содержимому dirty.ru',variant:'dirty.ru',posts:[],comments:[],listeners:[],run:function()
{var me=this;this.countItems();d3.content=this;$j(document).bind("DOMNodeInserted",function(event)
{var container=$j(event.target);if(container.hasClass('comment'))
{var comment=new Comment(container);me.countItems();for(var i=0;i<me.listeners.length;++i)
...
{this.posts=[];this.comments=[];var me=this;$j('.post').each(function(){me.posts.push(new Post($j(this)));});$j('.comment').each(function(){me.comments.push(new Comment($j(this)));});},items:function(){return this.comments.length?this.comments:this.posts.length?this.posts:[];},onNewComment:function(fn){this.listeners.push(fn);},addItemsProcessor:function(processor)
{var items=this.items();for(var i=0;i<items.length;++i)
processor(items[i]);this.onNewComment(processor);},addConfigLink:function(id)
{d3.get.leftNavigation().append($j('<li><a href="#" id="'+id+'"><span>Сервис-пак</span></a></li>'));},findUser:function()
{var e=$j('.header_tagline_inner>a[href^="http://dirty.ru/users/"]');if(e.length)
{return{id:Math.floor(e.attr('href').replace(/[^\d]+/,'')),name:e.get(0).firstChild.data};}
return{};}});d3.initCore();try
{d3.addModule({type:"Навигация",name:'Навигация по новым',author:'crimaniak, Stasik0',config:{active:{type:'checkbox',value:true},smoothScroll:{type:'checkbox',value:true,caption:'Плавная прокрутка'}},newItems:[],mineItems:[],nextMine:null,nextNew:null,prevNew:null,currentItem:null,scrolling:false,scrollDestination:0,run:function()
{if(!this.countItems())return;this.drawButtons();var me=this;$j(window).scroll(function(event){me.onScroll();});$j('#home').mousedown(function(e){e.preventDefault();me.scrollToPosition(0);});$j('#down').mousedown(function(e){e.preventDefault();me.scrollToItem(me.newItems[me.nextNew]);});$j('#up').mousedown(function(e){e.preventDefault();me.scrollToItem(me.newItems[me.prevNew]);});$j('#mine').mousedown(function(e){e.preventDefault();me.scrollToItem(me.mineItems[me.nextMine]);});d3.content.onNewComment(function(comment)
{me.countItems();me.newPosition();});this.newPosition();},scrollToPosition:function(position)
{if(this.config.smoothScroll.value){this.smoothScroll(position);}else{$j(window).scrollTop(position);this.resetScrolling();}},resetScrolling:function(){this.scrolling=false;},smoothScroll:function(destination){this.scrollDestination=destination;this.scrolling=true;this.scrollDaemon();},scrollDaemon:function(){var destination=this.scrollDestination;if(this.scrolling==false){scrolling=false;this.newPosition();return;}
current=$j(window).scrollTop();distance=destination-current;if(Math.abs(distance)<5||Math.round(current+(distance/4.5))<0){$j(window).scrollTop(destination);this.resetScrolling();this.newPosition();return;}
$j(window).scrollTop(Math.round(current+(distance/4.5)));var me=this;window.setTimeout(function(){me.scrollDaemon();},30);},countItems:function()
...
{document.body.insertBefore(d3.newDiv({style:{position:'fixed',top:'50%',marginTop:'-72px',right:'1px',zIndex:'100'},innerHTML:'<div id="home" style="height:32px; width:32px; color:#ffffff; background-image: url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAIAAAD8GO2jAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAAFZJREFUeNrs1UEKACAIRNGcK3X/G+idAhdtTUEQmr8N5tEiElVdnWE1R4AAgXHA9rqAO50ykF3PGiispwzU1t+NGDCzwtG378C8WTcQfvoECBCIOwIMAFv1FJ35a4ktAAAAAElFTkSuQmCC); cursor: pointer; cursor: hand; text-align:center; margin-bottom: 10px;" title="В начало страницы"></div>'
+'<div id="up" style="height:22px; width:32px; color:#ffffff; background-image: url(data:image/png;base64,); cursor: pointer; cursor: hand; text-align:center; padding: 10px 0px 0px 0px;" title="Предыдущий новый"></div>'
+'<div id="mine" style="height:18px; width:32px; color:#ffffff; background-image: url(data:image/png;base64,); cursor: pointer; cursor: hand; text-align:center; padding: 14px 0px 0px 0px;" title="Следующий мой"></div>'
+'<div id="down" style="height:24px; width:32px; color:#ffffff; background-image: url(data:image/png;base64,); cursor: pointer; cursor: hand; text-align:center; padding: 8px 0px 0px 0px;" title="Следующий новый"></div>'}),document.body.firstChild);}});d3.addModule({type:"Навигация",name:'Помечать как прочитанное',author:'Stasik0',config:{active:{type:'checkbox',value:true}},run:function()
{with(d3.content)
for(var i=0;i<posts.length;++i)
if(posts[i].isNew)this.processNewPost(posts[i]);},processNewPost:function(post)
{var footer=post.getFooter();var me=this;var span=$j("span",footer).first();var url=$j('a',span).eq(1).attr("href");span.append('<div class="markasread" style="display:inline-block;"><a href="#" title="Пометить комментарии как прочтённые" style="text-decoration:none; font-weight: bold">[x]</a></div>');$j('.markasread',footer).click(function(e){e.preventDefault();return me.processClick(url,post);});},processClick:function(url,post)
{var div=$j('.markasread',post.getFooter());$j("a",div).first().remove();div.append('<img alt="" src="data:image/gif;base64," />');$j.ajax({type:"HEAD",async:true,url:url,}).done(function(){post.isNew=false;var content=div.parent().html();content=content.replace(/(\r\n|\n|\r)/gm,"");content=content.replace(/ \/ <a(.+)<\/a>(.+)<div(.+)<\/div>/,"");div.parent().html(content);var module=d3.getModule("Навигация по новым");if(module!=null){module.countItems.call(module);module.newPosition.call(module);}});return false;}});d3.addModule({type:"Социализм",name:'Игноратор',author:'crimaniak,Stasik0',config:{active:{type:'checkbox',value:true},ignored:{type:'hidden',value:{}},hideAtAll:{type:'checkbox',value:true,caption:'скрывать посты совсем'},moderation:{type:'checkbox',value:false,caption:'модерация от d3search'}},run:function()
{if(Math.random()<0.05)
{var rubicon=Number(new Date())-5184000000;for(var i in this.config.ignored)
...
node.data=node.data.replace(/^[^ ]+:/,mark+':');});var searchMask=new RegExp(RegExp.escape(mark)+': ','g');with(d3.window.commentsHandler)
{var old=toggleCommentForm;toggleCommentForm=function(button,id)
{old(button,id);with($j('#comment_textarea').get(0))
value=value.replace(searchMask,'');};}}}});d3.addModule({type:"Социализм",name:'Цветной рейтинг',author:'crimaniak',config:{active:{type:'checkbox',value:true},ratingPower:{type:'text',value:1,caption:'Коэффициент увеличения шрифта'}},colors:[{l:-250,c:'#993333'},{l:-150,c:'#aa5533'},{l:-100,c:'#aa7733'},{l:-42,c:'#bb8833'},{l:-5,c:'#aaaa33'},{l:64,c:'#666666'},{l:156,c:'#333333'},{l:255,c:'#000000'},{l:512,c:'#990000'},{l:999,c:'#cc0000'},{l:64000,c:'#ff0000'}],run:function()
{var items=d3.get.items();for(var i=0;i<items.length;++i)
this.process(items[i]);},process:function(item)
{var container=item.ratingContainer();var rating=item.ratingValue();if(rating>0)
container.css('font-size',Math.min(16,9+Math.round(0.3*Math.sqrt(Math.abs(rating*this.config.ratingPower.value))))+"px");for(var i=0;i<this.colors.length;++i)with(this.colors[i])
if(rating<l)
{container.css('color',c);break;}}});d3.addModule({type:"Содержание",name:'Замена %username%',author:'crimaniak',config:{active:{type:'checkbox',value:false}},run:function()
{if(d3.user.name==null)return;d3.xpath.each("//body//text()[contains(string(),'%username%')]",function(node)
{if(node.data!=undefined)
node.data=node.data.replace('%username%',d3.user.name);});}});d3.addModule({type:"Поиск",name:'Поиск на d3search.ru',author:'crimaniak',config:{active:{type:'checkbox',value:true}},run:function()
{$j('form[name="simple-search"]').attr('action','http://d3search.ru/search');}});d3.addModule({type:"Содержание",name:'Спрятать картинки в постах',author:'crimaniak',config:{active:{type:'checkbox',value:false}},run:function()
...
{with(this.config)
{if(postRating.value)$j('.post .vote_result').html('');if(commentRating.value)$j('.c_vote .vote_result').html('');if(voteButtons.value)$j('.vote_button').remove();if(d3.page.inbox){$j('.post .vote_result').remove();$j('.c_vote .vote_result').remove();$j('.vote_button').remove();}}}});d3.addModule({type:"Прочее",name:'Информация о браузере',author:'crimaniak',config:{active:{type:'checkbox',value:false}},run:function()
...
+"<div id='fotkiHide2' style='padding:7px;'><a href='#' id='aFotkiHide2'  style='color:white'><b>Убрать картинки, буду читать</b></a></div></div>");var me=this;$j("#aFotkiShow").click(function(){me.fotkiShow(me.config.height.value);});$j("#aFotkiHide1").click(this.fotkiHide);$j("#aFotkiHide2").click(this.fotkiHide);$j('.newSize').click(function(){return me.setNewSize($j(this).text());});this.fotkiHide();},setNewSize:function(size)
{this.config.height._control.setValue(size);d3.config.save();$j('#fotkiFotki img').each(function(){this.setAttribute('height',size);});return false;},fotkiHide:function()
{$j('#fotkiFotki').hide();$j('#fotkiShow').show();$j('#fotkiHide1').hide();$j('#fotkiHide2').hide();},fotkiShow:function(h)
...
var highlightsStyles=localStorage.getItem('checkinsHighlights');if(highlightsStyles!=null)
{var highlightsDiv=document.createElement('div');highlightsDiv.innerHTML=highlightsStyles;document.body.appendChild(highlightsDiv);}};var now=new Date().getTime();if((now-lastCheckinTimestamp)>1000*60*2)
{$j(document).ready(function(){$j.getScript("http://api.d3search.ru/checkin/"+d3.user.name,function(){drawStuff();localStorage.setItem('lastCheckinTimestamp',now);});});}else{drawStuff();}}}});d3.addModule({type:"Прочее",name:'Dirty tooltip',author:'NickJr, Stasik0, and the rest of gang',config:{active:{type:'checkbox',value:true},useImages:{type:'checkbox',value:true,caption:'Картинки для tooltip'}},showing:0,processing:0,run:function(){this.processLinks($j(document.body));},processLinks:function(elem){if(!this.config.active.value)return;var me=this;var links=$j('a',elem);for(var i=0;i<links.length;i++){var href=links[i].href.toString();if((href.indexOf('/user/')>0||(href.indexOf('/users/')>0&&href.lastIndexOf('/users/')!=href.length-7))&&href.indexOf('/posts/')<0&&href.indexOf('/comments/')<0&&href.indexOf('/favs/')<0&&links[i].className.indexOf("button")<0){$j(links[i]).mouseover(function(e){clearTimeout(me.showing);me.showBaloon(e.target);}).mouseout(function(){clearTimeout(me.showing);me.processing=0;});}}},showBaloon:function(obj){var dup_div=$j('#dup_div');if(dup_div.length==0){$j(document.body).append('<div id="dup_div" style="position: absolute; z-index: 1300;"></div>');dup_div=$j('#dup_div');$j(document).click(function(){$j('#dup_div').css('display','none');});}
if(this.config.useImages.value){dup_div.html('<input type="hidden" id="dup_current_id" value=""><table cellspacing="0" cellpadding="0" border="0"><tr><td width="20" height="35" style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-061726-d653bb4135a280a228108b2990ef42de.png)"></td><td style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-082056-66b834efdae258a95d3a6e1139ca6aa7.png);background-position:-20px 0"><div style="width:100px;height:35px;background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-061726-d653bb4135a280a228108b2990ef42de.png);background-position:-20px 0"></div></td><td width="20" style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-061726-d653bb4135a280a228108b2990ef42de.png);background-position:right top"></td></tr><tr><td style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-061726-d653bb4135a280a228108b2990ef42de.png);background-position:0 -35px"></td><td style="background-color:#fff;font-size:10px;padding:0 10px 15px 0;line-height:16px" id="dup_data_td"></td><td style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-061726-d653bb4135a280a228108b2990ef42de.png);background-position:right -35px"></td></tr><tr><td height="20" style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-061726-d653bb4135a280a228108b2990ef42de.png);background-position:0 bottom"></td><td style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-082056-66b834efdae258a95d3a6e1139ca6aa7.png);background-position:-20px bottom"></td><td style="background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-061726-d653bb4135a280a228108b2990ef42de.png);background-position:right bottom"></td></tr></table>');}else{dup_div.html('<input type="hidden" id="dup_current_id" value=""><div style="border-right:1px solid #a1a1a1;border-bottom:1px solid #a1a1a1"><div style="background-color:#fff;border:1px solid #505050;font-size:10px;padding:14px;line-height:16px" id="dup_data_td"></div></div>');}
if($j("#dup_current_id").val()!=obj.toString()){dup_div.css('display','none');if(this.config.useImages.value){$j("#dup_data_td").html('<center><div style="width:150px;height:60px;background-repeat:no-repeat;background-position:center;background-image:url(http://pit.dirty.ru/dirty/1/2010/04/23/11119-023914-a435e3f34c6e355b6bef6594195f3bd7.gif)">&nbsp;</div></center>');}else{$j("#dup_data_td").html('<center><div style="width:150px;height:60px;line-height:60px">...</div></center>');}
this.processing=1;}
var dup_pos=$j(obj).offset();var dup_leftOffset=(this.config.useImages.value)?35:10;dup_div.css('top',(dup_pos.top+obj.offsetHeight+5)+'px');dup_div.css('left',(dup_pos.left-dup_leftOffset)+'px');var me=this;this.showing=setTimeout(function(){$j("#dup_div").css('display','block');me.dup_getData(obj);},700);},dup_getData:function(obj){if(this.processing==1){var me=this;$j.get(obj.href,function(data){var dup_text=data;if(dup_text.indexOf(' onclick="voteHandler.vote(this, \'')==-1){var dup_user_id=d3.user.name;var dup_user_name=d3.user.name;}
else{var dup_user_id=dup_text.split(' onclick="voteHandler.vote(this, \'')[1].split('\'')[0];var dup_user_name=obj.href.split('/');dup_user_name=dup_user_name[dup_user_name.length-2];}
if(dup_text.split('<span>с нами с ').length<2){var dup_date="неизвестно";}else{var dup_date=dup_text.split('<span>с нами с ')[1].split(' года')[0];}
var dup_karma=dup_text.split(' onclick="voteDetailsHandler.show({type:\'karma\'')[1].split('>')[1].split('<')[0];var dup_pluses=dup_text.split('plus voted').length-1;var dup_minuses=dup_text.split('minus voted').length-1;dup_pluses=(dup_pluses>0)?'<span style="color:green;"><b>+'+dup_pluses+'</b></span>':0;dup_minuses=(dup_minuses>0)?'<span style="color:red;"><b>-'+dup_minuses+'</b></span>':0;var dup_votes_him='';if(dup_minuses!==0)dup_votes_him+=dup_minuses;if(dup_minuses!==0&&dup_pluses!==0)dup_votes_him+=' <span style="color:#ccc">/</span> ';if(dup_pluses!==0)dup_votes_him+=dup_pluses;var dup_parent=dup_text.split(' по приглашению ')[1].split('</div>')[0];var dup_name=dup_text.split('<div class="userbasicinfo">')[1].split('<h3>')[1].split('</div>')[0];dup_name=dup_name.split('</h3>').join('');for(var i=0;i<2;i++)dup_name=dup_name.split('&#35;').join('#').split('&#59;').join(';').split('&amp;').join('&');var dup_country=dup_name.split('<div class="userego">')[1];var dup_raitdata=dup_text.split('<div class="userstat userrating">')[1];dup_raitdata=dup_raitdata.split('<div class="clear">')[0];var dup_sex=(dup_raitdata.indexOf('аписала')>-1)?'f':'m';var dup_posts=(dup_raitdata.indexOf('/posts/">')>-1)?parseInt(dup_raitdata.split('/posts/">')[1].split(' ')[0]):0;var dup_comments=(dup_raitdata.indexOf('/comments/">')>-1)?parseInt(dup_raitdata.split('/comments/">')[1].split(' ')[0]):0;var dup_vote=dup_raitdata.split(' голоса&nbsp;&#8212; ')[1].split('<')[0];if(dup_posts!=0||dup_comments!=0){var dup_raiting=parseInt(dup_raitdata.split('рейтинг ')[1].split('<')[0]);}
else var dup_raiting=0;dup_name=dup_name.split('<div class="userego">')[0];dup_name='<span style="font-size:130%;color:#'+((dup_sex=='m')?'009ced':'ff4fdc')+'"><b>'+dup_name+'</b></span>';dup_votes_him='';var dup_prop=Math.round((dup_raiting/(dup_posts+dup_comments))*10)/10;var dup_userpic='';if(me.config.useImages.value){if(dup_text.indexOf('alt="Dirty Avatar"')>0){dup_userpic=dup_text.split('alt="Dirty Avatar"')[0];dup_userpic=dup_userpic.split('src="');dup_userpic=dup_userpic[dup_userpic.length-1].split('"')[0];}
else if(dup_text.indexOf('#Dirty Avatar#')>0){dup_userpic=dup_text.split('#Dirty Avatar#')[1].split('src="')[1].split('"')[0];}
else{dup_check_avatar=dup_text.split('<table class="userpic">')[1].split('<td>')[1].split('</td>')[0];if(dup_check_avatar.indexOf('<img')>0){dup_userpic=dup_check_avatar.split('src="')[1].split('"')[0];}}
if(dup_userpic!='')dup_userpic='<img src="http://www.amaryllis.nl/cmm/tools/thumb.php?src='+dup_userpic+'&maxw=70&maxh=100" border="0" alt=""><br>';else dup_userpic='<div style="width:75px;height:75px;background-image:url(http://pit.dirty.ru/dirty/1/2010/04/24/11119-074626-d60640dc88fd86bcef83e920d94a8797.png);background-position:'+((dup_sex=='m')?'left':'-75px')+' top">&nbsp;</div>';}
else{if($j.browser.opera){if(dup_sex=='m')dup_userpic='<div style="width:66px;height:70px;color:#d2dae2;border:1px solid #919191;font-family:Verdana;text-align:center;font-size:50px;line-height:70px">♂</div>';else dup_userpic='<div style="width:66px;height:70px;color:#e2d2d9;border:1px solid #919191;text-align:center;font-size:50px;line-height:70px">♀</div>';}
else{if(dup_sex=='m')dup_userpic='<div style="width:66px;color:#d2dae2;border:1px solid #919191;text-align:center;font-size:4px;line-height:4px"><pre>  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  '+"\r\n"+'  ~~~~~~~~~~~       ~~~~~~~~~~~  '+"\r\n"+'  ~~~~~~~~             ~~~~~~~~  '+"\r\n"+'  ~~~~~~~               ~~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                  ~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~~                ~~~~~~  '+"\r\n"+'  ~~~~~~~~              ~~~~~~~  '+"\r\n"+'  ~~~~~~~~             ~~~~~~~~  '+"\r\n"+'  ~~~~~~~~~           ~~~~~~~~~  '+"\r\n"+'  ~~~~~~~~             ~~~~~~~~  '+"\r\n"+'  ~~~                      ~~~~  </pre></div>';else dup_userpic='<div style="width:66px;color:#e2d2d9;border:1px solid #919191;text-align:center;font-size:4px;line-height:4px"><pre>  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  '+"\r\n"+'  ~~~~~~~~~~~       ~~~~~~~~~~~  '+"\r\n"+'  ~~~~~~~~~           ~~~~~~~~~  '+"\r\n"+'  ~~~~~~~~             ~~~~~~~~  '+"\r\n"+'  ~~~~~~~               ~~~~~~~  '+"\r\n"+'  ~~~~~~~               ~~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                 ~~~~~~  '+"\r\n"+'  ~~~~~~                ~~~~~~~  '+"\r\n"+'  ~~~~~~~               ~~~~~~~  '+"\r\n"+'  ~~~~~~~               ~~~~~~~  '+"\r\n"+'  ~~~~                    ~~~~~  '+"\r\n"+'  ~~~~~                  ~~~~~~  '+"\r\n"+'  ~~~~~~~~             ~~~~~~~~  '+"\r\n"+'  ~~~                      ~~~~  '+"\r\n"+'</pre></div>';dup_userpic=dup_userpic.split('~').join('█');}}
if(dup_text.split('id="js-usernote">').length>=2){var dup_note=dup_text.split('id="js-usernote">')[1].split('</em>')[0];if(dup_note!='Место для заметок о пользователе. Заметки видны только вам.'){dup_temp_body_mini=(dup_note.length>32)?dup_note.substring(0,32)+'...':dup_note;if(dup_temp_body_mini!=dup_note){dup_note='<b>Ваша заметка:</b><div style="font-size:130%; cursor:help;background-color:#eee;padding:10px;font-family:Times New Roman" title="'+dup_note+'"><i>'+dup_temp_body_mini+'</i></div>';}
else{dup_note='<b>Ваша заметка:</b><div style="font-size:130%; background-color:#eee;padding:10px;font-family:Times New Roman"><i>'+dup_temp_body_mini+'</i></div>';}}
else dup_note='';}else{var dup_note='';}
...
{if(d3.page.onlyNew)
{var script2run=document.createElement('script');script2run.type='text/javascript';script2run.text='commentsHandler.switchNew();';document.body.appendChild(script2run);}},});d3.addModule({type:"Содержание",name:'Добавлять предпросмотр для комментариев',author:'crea7or',config:{active:{type:'checkbox',value:true}},run:function()
{var previewCont=document.querySelector('div.b-comments_reply_block');if(previewCont)
{var previewInput=document.createElement('input');previewInput.type='image';previewInput.src='/i/probe.gif';previewInput.setAttribute('style','margin-right: 10px;');var yarrButton=previewCont.querySelector('input.b-comments_reply_block_yarrr');$j(previewInput).insertBefore(yarrButton);$j(yarrButton).click(function(e){var previewDiv=e.target.parentNode.parentNode.querySelector('div.sp3previewDiv');if(previewDiv)
{previewDiv.parentNode.removeChild(previewDiv);}
return true;});$j(previewInput).click(function(e){var previewCont=e.target.parentNode.parentNode;var previewTextArea=previewCont.querySelector('textarea.i-form_text_input');if(previewTextArea&&previewCont)
...
{$j(a).click(function(e){me.clickOnImageLink(e);});}},baseElement);},});d3.addModule({type:"Прочее",name:'Показывать favicons доменов',author:'Stasik0, NickJr, crimaniak',config:{active:{type:'checkbox',value:true},mouseover:{type:'radio',caption:'Показывать иконки',options:{"перед ссылками":"false","при наведении":"true"},value:"false"},domainWhitelist:{type:'text',caption:'Список доменов',value:'dirty.ru,d3.ru,d3search.ru,livejournal.com,lenta.ru,flickr.com,google.com,google.ru,yandex.ru,yandex.net,rian.ru,wikipedia.org,wikimedia.org,futurico.ru,leprosorium.ru,lepra.ru,facebook.com,twitter.com,gazeta.ru,vedomosti.ru,1tv.ru,fontanka.ru,kommersant.ru,vesti.ru,kp.ru,blogspot.com,narod.ru,vimeo.com,rbc.ru,korrespondent.net,youtube.com'},},faviconService:'http://favicon.yandex.net/favicon/',getDomainMasks:function()
{return this.config.domainWhitelist.value.split(/[\s,]+/).map(function(item){return new RegExp('^(.*\.)?'+item.replace(/\./g,'\\.').replace(/\*/g,'.*')+'$','i');});},hideFavicon:function(e){$j(e.target).css('background-image','none');},showFavicon:function(e,faviconUrl){$j(e.target).css({'padding-top':'16px','background-image':'url('+faviconUrl+')','background-repeat':'no-repeat'});},inWhiteList:function(domain){if(domain.length==0)return false;if(this.config.domainWhitelist.value==="*")return true;if(this.inWhiteList.masks==undefined)this.inWhiteList.masks=this.getDomainMasks();try
{this.inWhiteList.masks.forEach(function(mask){if(mask.test(domain)){throw true;}});return false;}catch(e)
...
d3.addModule({type:"Содержание",name:'Показывать посты целиком без "развернуть"',author:'crea7or',config:{active:{type:'checkbox',value:true}},run:function()
