...
d3.addContentModule(/(.*\.)?leprosorium.ru/i,
{
	...
	run: function()
	{
		...
		d3.page=
		{
			...
			inboxComments: (( document.location.pathname.indexOf("/inbox/") > 0 ) && ( window.location.pathname.length > 24 )),
		...
		d3.get =
		{
			logoutLink: function(){return $j('#logout');},
			leftNavigation: function(){return $j('.layout_left');},
	...
	addToLeftNav: function(item, after)
	{
		...
		if(after !== undefined)
		{
			after = $j('li:contains("'+after+'")',nav);
			nav.insertAfter(after);
		} else
			nav.append(item);
	},
	...
	{	
		if(top)
			this.addToHeaderNav($j('<li><a id="'+id+'" href="#" class="b-fui_icon_button b-fui_icon_button_settings" title="Настройки сервис-пака"><span></span><em></em></a></li>'));
		else
	...
	{
		var user = { name: "", id: 0};
		if ( d3.window.chatHandler )
		{
			user.name = d3.window.chatHandler.username;
		}
		return user;
	},
	...
	{
		$j('div.layout_left').append('<div id="leftNavigator"><ul id="leftNavUl" style="list-style-type: none; padding: 0px 0px 0px 5px; font-size: 0.85em;"></ul></div>');
	}	
});
